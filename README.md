...
User
 └──> Frontend (HTML/CSS/JS)
        └──> Flask Server (Python)
               └──> Plagiarism Detector Engine (C)
                      └──> AST / CFG / DAG Analysis
                             └──> Result JSON
                                    └──> Rendered as Visual Report


🧠 Working Process:
1️⃣ User Uploads Files
  Multiple .c files, folder, or .zip archive.
2️⃣ Flask Backend Saves Files
  Files extracted in a temporary /uploads folder.
3️⃣ Backend Executes C Engine
  Runs the compiled plagiarism_detector.exe program.
4️⃣ C Engine Performs Deep Analysis
  Builds AST, CFG, and DAG for each file pair.
  Calculates structural similarity metrics.
5️⃣ Flask Converts Results into JSON
  Structured output returned to frontend.
6️⃣ Frontend Displays Colored Results
  Each file pair shown in a card with similarity % and detailed breakdown.
...
Results Section
Each comparison shown in clean card:
...
🌟 Star on GitHub — it motivates me to build more amazing projects 💻❤️
